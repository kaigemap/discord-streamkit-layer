...
`),p+=`${k}
`,t.onlyRegistered&&c.id.startsWith("unset_")&&(p+=`li[class*="Voice_voiceState"]:has(img[src*="${f}"]) {
  display: none !important;
}
`)}),t.hideNames&&(p+=`
/* Global Hide Names */
...
}`;default:return""}}function Y(t){t.innerHTML=`
    <div class="Voice_voiceContainer voice_container">
      <ul class="Voice_voiceStates voice_states" id="simulator-list">
        <!-- Injected via state -->
      </ul>
    </div>
  `}function W(t){const e=document.getElementById("simulator-list");e&&(e.innerHTML=t.map((n,o)=>{const a=n.name||`User ${o+1}`,r=n.displayName&&n.displayName.trim()!==""?n.displayName:a;return`
    ...
    `).join(""),[s,l]=t.content.includes("/* --- User Highlight --- */")?t.content.split("/* --- User Highlight --- */"):[t.content,""];o.innerHTML=`
      <style>
        :host {
          display: flex;
          align-items: center;
    ...
    `,e.appendChild(n);const m=document.createElement("span");m.className="theme-name",m.textContent=t.name,e.appendChild(m),e.addEventListener("click",()=>{i.themeId=t.id,document.querySelectorAll(".theme-card").forEach(v=>v.classList.remove("active")),e.classList.add("active"),d()}),$.appendChild(e)})}function g(){y.innerHTML="",[...i.users].map((e,n)=>({...e,originalIndex:n})).sort((e,n)=>(e.priority??0)-(n.priority??0)).forEach((e,n)=>{const o=document.createElement("div");o.className="user-item user-table-grid",o.innerHTML=`
    ...
    `,y.appendChild(o)}),y.querySelectorAll("input").forEach(e=>{e.addEventListener("input",n=>{const o=n.target.dataset.type,a=n.target.dataset.userid,r=i.users.findIndex(l=>l.id===a);if(r===-1)return;let s=n.target.value;o==="priority"&&(s=parseInt(s)||0),i.users[r][o]=s,o==="priority"&&g(),d()})}),y.querySelectorAll(".remove-user").forEach(e=>{e.addEventListener("click",n=>{const o=n.target.dataset.userid,a=i.users.findIndex(r=>r.id===o);a!==-1&&(i.users.splice(a,1),g(),d())})}),y.querySelectorAll(".visibility-toggle").forEach(e=>{e.addEventListener("click",n=>{const o=n.target.dataset.userid||n.target.parentElement.dataset.userid,a=i.users.findIndex(r=>r.id===o);a!==-1&&(i.users[a].isHidden=!i.users[a].isHidden,g(),d())})})}function U(){const t=u[i.language];document.querySelectorAll("[data-i18n]").forEach(e=>{const n=e.dataset.i18n;if(t[n]){if(e.querySelector(".material-symbols-rounded"))return;e.textContent=t[n]}}),window.copiedMessage=t.copiedAction}function A(){const t=u[i.language];document.getElementById("copy-css").title=t.copyCss,document.getElementById("download-css").title=t.saveCss,document.getElementById("export-json").title=t.saveConfig,document.getElementById("import-json-btn").title=t.loadConfig}function T(){document.getElementById("avatar-size").value=i.avatarSize,document.getElementById("avatar-size-slider").value=i.avatarSize,document.getElementById("base-font-size").value=i.baseFontSize,document.getElementById("base-font-size-slider").value=i.baseFontSize,document.getElementById("layout-gap").value=i.gap,document.getElementById("layout-gap-slider").value=i.gap,document.getElementById("unset-count").value=i.unsetUserCount,document.getElementById("unset-count-slider").value=i.unsetUserCount;function t(o,a){const r=document.getElementById(o);r&&r.querySelectorAll(".toggle-btn").forEach(s=>{s.dataset.value===a?s.classList.add("active"):s.classList.remove("active")})}t("anim-type-group",i.animType),t("layout-direction-group",i.direction),t("layout-wrap-group",i.wrap),t("layout-align-group",i.justifyContent),document.getElementById("hide-names").checked=i.hideNames;const e=document.getElementById("default-color");e&&(e.value=i.defaultColor||"#ffffff");const n=document.getElementById("unlisted-toggle-group");if(n){const o=i.onlyRegistered?"hide":"show";n.querySelectorAll(".toggle-btn").forEach(a=>{a.dataset.value===o?a.classList.add("active"):a.classList.remove("active")})}document.querySelectorAll(".lang-btn").forEach(o=>{o.classList.toggle("active",o.dataset.lang===i.language)})}function d(){const e=[...[...i.users].sort((l,m)=>(l.priority??0)-(m.priority??0))],n=[];for(let l=0;l<i.unsetUserCount;l++)n.push({id:`unset_${l}`,name:`Unset${l+1}`,color:i.defaultColor,priority:100+l,isHidden:!1});e.push(...n);const o=w(i);let a=document.getElementById("generated-styles");a||(a=document.createElement("style"),a.id="generated-styles",document.head.appendChild(a)),a.textContent=o;let r=document.getElementById("preview-styles");if(!r){r=document.createElement("style"),r.id="preview-styles";const l=document.getElementById("discord-simulator");l&&l.appendChild(r)}r&&(r.textContent=o);const s=document.getElementById("css-output");s&&(s.textContent=o),i.displayedUsers=e,W(e)}Q();

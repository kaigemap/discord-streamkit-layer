...
img[src*="${l.id}"],
img[src*="${l.id}"] + div[class^="Voice_user"] {
  display: none !important;
}
...
@keyframes shake-anim {
  ...
  `}function Y(t){const e=document.getElementById("simulator-list");e&&(e.innerHTML=t.map((n,o)=>{const a=n.name||`User ${o+1}`,r=n.displayName&&n.displayName.trim()!==""?n.displayName:a;return`
      <li class="Voice_voiceState voice_state" data-userid="${n.id}">
        ...
        <div class="Voice_user voice_user">
          <span class="Voice_name voice_name">${r}</span>
        </div>
      </li>
    `}).join(""))}function V(t,e){const n=document.querySelector(`#simulator-list [data-userid="${t}"]`);if(n){const o=n.querySelector(".Voice_avatar");o&&(e?(n.classList.add("wrapper_speaking"),o.classList.add("Voice_avatarSpeaking")):(n.classList.remove("wrapper_speaking"),o.classList.remove("Voice_avatarSpeaking")))}}function J(){const t=document.getElementById("simulator-list");t&&t.classList.toggle("show-metadata")}let i={users:[{id:"12345678901",displayName:"",color:"#ff4b4b",priority:0},{id:"12345678902",displayName:"",color:"#4b4bff",priority:1}],animType:"bounce",baseFontSize:14,avatarSize:100,gap:0,direction:"row",wrap:"nowrap",justifyContent:"flex-start",themeId:"horizontal",language:"ja",onlyRegistered:!1,hideNames:!1,defaultColor:"#ffffff",unsetUserCount:3,displayedUsers:[],isOverlayMode:new URLSearchParams(window.location.search).get("mode")==="overlay"};i.isOverlayMode&&document.body.classList.add("overlay-active");const f=document.getElementById("user-list"),z=document.getElementById("theme-selector"),W=document.getElementById("discord-simulator");function Q(){X(),Z(),K(),setTimeout(()=>requestAnimationFrame(()=>c()),0)}function X(){U(),N(),I(),m(),G(W),A();const t=document.getElementById("toggle-unset-users");t.textContent=`Unset Users (${i.unsetUserCount})`}function Z(){ee(),te(),ne(),ae(),ie(),oe()}function K(){function t(){const e=Array.from(document.querySelectorAll("#simulator-list [data-userid]")).map(n=>{const o=n.dataset.userid,a=i.displayedUsers.find(r=>r.id===o);return a&&!a.isHidden?a:null}).filter(Boolean);if(e.length>0){const n=e[Math.floor(Math.random()*e.length)];V(n.id,!0),$(n.id,!0);const o=1200+Math.random()*1800;setTimeout(()=>{V(n.id,!1),$(n.id,!1);const a=400+Math.random()*1e3;setTimeout(t,a)},o)}else setTimeout(t,1e3)}t()}function $(t,e){document.querySelectorAll(".theme-card").forEach(n=>{var a;const o=(a=n.querySelector(".theme-preview-container"))==null?void 0:a.shadowRoot;if(o){const r=o.querySelector('[data-userid="mini1"]'),s=r==null?void 0:r.querySelector(".Voice_avatar");r&&s&&(e?(r.classList.add("wrapper_speaking"),s.classList.add("Voice_avatarSpeaking")):(r.classList.remove("wrapper_speaking"),s.classList.remove("Voice_avatarSpeaking")))}})}function ee(){document.getElementById("add-user").addEventListener("click",()=>{i.users.push({id:Date.now().toString(),displayName:"",color:"#ffffff",priority:i.users.length}),m(),c()})}function y(t,e){const n=document.getElementById(t);n&&n.querySelectorAll(".toggle-btn").forEach(o=>{o.addEventListener("click",a=>{n.querySelectorAll(".toggle-btn").forEach(s=>s.classList.remove("active")),a.target.classList.add("active");const r=a.target.dataset.value;i[e]=e==="onlyRegistered"?r==="hide":r,c()})})}function te(){y("anim-type-group","animType"),y("layout-direction-group","direction"),y("layout-wrap-group","wrap"),y("layout-align-group","justifyContent"),y("unlisted-toggle-group","onlyRegistered")}function S(t,e,n){const o=document.getElementById(t),a=document.getElementById(e),r=s=>{if(i[n]=parseInt(s)||0,o.value=i[n],a.value=i[n],c(),n==="unsetUserCount"){const u=document.getElementById("toggle-unset-users");u.textContent=`Unset Users (${i.unsetUserCount})`}};o.addEventListener("input",s=>r(s.target.value)),a.addEventListener("input",s=>r(s.target.value))}function ne(){S("avatar-size","avatar-size-slider","avatarSize"),S("base-font-size","base-font-size-slider","baseFontSize"),S("layout-gap","layout-gap-slider","gap"),S("unset-count","unset-count-slider","unsetUserCount"),(()=>{const e=document.getElementById("toggle-unset-users");e.textContent=`Unset Users (${i.unsetUserCount})`})(),document.getElementById("hide-names").addEventListener("change",e=>{i.hideNames=e.target.checked,c()})}function ae(){document.querySelectorAll(".lang-btn").forEach(t=>{t.addEventListener("click",e=>{i.language=e.target.dataset.lang,document.querySelectorAll(".lang-btn").forEach(n=>n.classList.remove("active")),e.target.classList.add("active"),U(),N(),I(),m(),c()})})}function ie(){document.querySelectorAll(".sidebar-tab").forEach(t=>{t.addEventListener("click",e=>{const n=e.target.dataset.sidebarTab;document.querySelectorAll(".sidebar-tab").forEach(a=>a.classList.remove("active")),e.target.classList.add("active"),document.querySelectorAll(".sidebar-scroll").forEach(a=>{a.classList.remove("active"),a.style.display="none"});const o=document.getElementById(`sidebar-tab-${n}`);o.classList.add("active"),o.style.display="flex",n==="css"&&(document.getElementById("css-output").textContent=b(i))})})}function oe(){document.getElementById("toggle-debug").addEventListener("click",t=>{t.currentTarget.classList.toggle("active"),J()}),document.getElementById("toggle-unset-users").addEventListener("click",()=>{document.getElementById("unset-users-control").classList.toggle("hidden")}),document.getElementById("copy-css").addEventListener("click",()=>{const t=b(i);navigator.clipboard.writeText(t).then(()=>{const e=d[i.language].copiedAction||"Copied!";x(e)})}),document.getElementById("download-css").addEventListener("click",()=>{const t=b(i),e=new Blob([t],{type:"text/css"}),n=URL.createObjectURL(e),o=document.createElement("a");o.href=n,o.download="overlay.css",o.click(),URL.revokeObjectURL(n)}),document.getElementById("export-json").addEventListener("click",()=>{const t=new Blob([JSON.stringify(i,null,2)],{type:"application/json"}),e=URL.createObjectURL(t),n=document.createElement("a");n.setAttribute("href",e),n.setAttribute("download","overlay-config.json"),document.body.appendChild(n),n.click(),n.remove(),URL.revokeObjectURL(e)}),document.getElementById("import-json-btn").addEventListener("click",()=>{document.getElementById("import-json-input").click()}),document.getElementById("import-json-input").addEventListener("change",t=>{const e=t.target.files[0];if(!e){console.log("No file selected");return}const n=new FileReader;n.onload=o=>{try{console.log("File loaded, parsing JSON...");const a=JSON.parse(o.target.result);console.log("Parsed config:",a),i={...i,...a},console.log("State updated:",i),U(),A(),c(),I(),m(),x(d[i.language].loadConfig+" OK")}catch(a){console.error("JSON parse error:",a),x("Invalid JSON: "+a.message)}},n.readAsText(e),t.target.value=""})}function x(t){const e=document.createElement("div");e.className="notification",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.classList.add("show"),10),setTimeout(()=>{e.classList.remove("show"),setTimeout(()=>e.remove(),300)},2e3)}function I(){z.innerHTML="",C.forEach(t=>{const e=document.createElement("button");e.className=`theme-card ${i.themeId===t.id?"active":""}`,e.dataset.theme=t.id;const n=document.createElement("div");n.className="theme-preview-container";const o=n.attachShadow({mode:"open"}),r=[{id:"mini1",color:"#ff4b4b"}].map((g,h)=>`
      <li class="Voice_voiceState voice_state" data-userid="${g.id}">
        <img class="Voice_avatar voice_avatar Voice_avatarSpeaking" src="https://cdn.discordapp.com/embed/avatars/${h%6}.png#id=${g.id}" />
        <div class="Voice_user voice_user">
          <span class="Voice_name voice_name">User</span>
        </div>
      </li>
    `).join(""),[s,u]=t.content.includes("/* --- User Highlight --- */")?t.content.split("/* --- User Highlight --- */"):[t.content,""];o.innerHTML=`
      <style>
        :host {
          display: flex;
          align-items: center;
        ...
        ${u.replace(/USER_ID/g,"mini1").replace(/var\(--user-color\)/g,"#ff4b4b").replace(/var\(--user-color-alpha\)/g,"rgba(255, 75, 75, 0.4)").replace(/{{avatarSize}}/g,i.avatarSize)}
    ...
    `,e.appendChild(n);const v=document.createElement("span");v.className="theme-name",v.textContent=t.name,e.appendChild(v),e.addEventListener("click",()=>{i.themeId=t.id,document.querySelectorAll(".theme-card").forEach(g=>g.classList.remove("active")),e.classList.add("active"),c()}),z.appendChild(e)})}function m(){f.innerHTML="",[...i.users].map((e,n)=>({...e,originalIndex:n})).sort((e,n)=>(e.priority??0)-(n.priority??0)).forEach((e,n)=>{const o=document.createElement("div");o.className=`user-item user-table-grid ${e.isHidden?"user-item-hidden":""}`,o.innerHTML=`
      <input type="text" value="${e.id}" data-type="id" data-userid="${e.id}" placeholder="${d[i.language].userId}" title="${d[i.language].userId}" />
      <input type="text" value="${e.displayName||""}" data-type="displayName" data-userid="${e.id}" placeholder="上書きする場合のみ入力" title="${d[i.language].displayName}" />
      <input type="number" value="${e.priority??n}" data-type="priority" data-userid="${e.id}" placeholder="0" title="${d[i.language].priority}" />
      <input type="color" value="${e.color}" data-type="color" data-userid="${e.id}" title="${d[i.language].userColor}" />

      <div class="user-actions">
        <button class="visibility-toggle ${e.isHidden?"hidden":""}" data-userid="${e.id}" title="${d[i.language].visibility}">
    ...
    `,f.appendChild(o)}),f.querySelectorAll("input").forEach(e=>{e.addEventListener("input",n=>{const o=n.target.dataset.type,a=n.target.dataset.userid,r=i.users.findIndex(u=>u.id===a);if(r===-1)return;let s=n.target.value;o==="priority"&&(s=parseInt(s)||0),i.users[r][o]=s,o==="priority"&&m(),c()})}),f.querySelectorAll(".remove-user").forEach(e=>{e.addEventListener("click",n=>{const o=n.target.dataset.userid,a=i.users.findIndex(r=>r.id===o);a!==-1&&(i.users.splice(a,1),m(),c())})}),f.querySelectorAll(".visibility-toggle").forEach(e=>{e.addEventListener("click",n=>{const o=n.target.dataset.userid,a=i.users.findIndex(r=>r.id===o);a!==-1&&(i.users[a].isHidden=!i.users[a].isHidden,m(),c())})})}function U(){const t=d[i.language];document.querySelectorAll("[data-i18n]").forEach(e=>{const n=e.dataset.i18n;if(t[n]){if(e.querySelector(".material-symbols-rounded"))return;e.textContent=t[n]}}),window.copiedMessage=t.copiedAction}function N(){const t=d[i.language];document.getElementById("copy-css").title=t.copyCss,document.getElementById("download-css").title=t.saveCss,document.getElementById("export-json").title=t.saveConfig,document.getElementById("import-json-btn").title=t.loadConfig}function A(){document.getElementById("avatar-size").value=i.avatarSize,document.getElementById("avatar-size-slider").value=i.avatarSize,document.getElementById("base-font-size").value=i.baseFontSize,document.getElementById("base-font-size-slider").value=i.baseFontSize,document.getElementById("layout-gap").value=i.gap,document.getElementById("layout-gap-slider").value=i.gap,document.getElementById("unset-count").value=i.unsetUserCount,document.getElementById("unset-count-slider").value=i.unsetUserCount;function t(o,a){const r=document.getElementById(o);r&&r.querySelectorAll(".toggle-btn").forEach(s=>{s.dataset.value===a?s.classList.add("active"):s.classList.remove("active")})}t("anim-type-group",i.animType),t("layout-direction-group",i.direction),t("layout-wrap-group",i.wrap),t("layout-align-group",i.justifyContent),document.getElementById("hide-names").checked=i.hideNames;const e=document.getElementById("default-color");e&&(e.value=i.defaultColor||"#ffffff");const n=document.getElementById("unlisted-toggle-group");if(n){const o=i.onlyRegistered?"hide":"show";n.querySelectorAll(".toggle-btn").forEach(a=>{a.dataset.value===o?a.classList.add("active"):a.classList.remove("active")})}document.querySelectorAll(".lang-btn").forEach(o=>{o.classList.toggle("active",o.dataset.lang===i.language)})}function c(){const e=[...[...i.users].sort((r,s)=>(r.priority??0)-(s.priority??0))];if(!i.onlyRegistered){const r=[];for(let s=0;s<i.unsetUserCount;s++)r.push({id:`unset_${s}`,name:`Unset${s+1}`,color:i.defaultColor,priority:100+s,isHidden:!1});e.push(...r)}const n=b(i);let o=document.getElementById("generated-styles");o||(o=document.createElement("style"),o.id="generated-styles",document.head.appendChild(o)),o.textContent=n;const a=document.getElementById("css-output");a&&(a.textContent=n),i.displayedUsers=e,Y(e)}Q();
